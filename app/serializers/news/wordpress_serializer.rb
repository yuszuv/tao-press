  ...
  module News
    class WordpressSerializer < TaoPress::Serializer
      ...
      serialize :content do |obj, base_url:|
        ...
        if res.length == 0
          res = T[:text_markup].(Entities::Content::Text.new(text: obj.excerpt, type: "text", headline: nil, image: nil, images: []), base_url)
        end
      ...
      def self.content_reducer(base_url:)
        -> (res, content) {
          ...
          in Entities::Content::Download
            T[:download_markup].(content, base_url)
          in Entities::Content::YouTube
            T[:youtube_markup].(content)
          in Entities::Content::Image
            T[:image_markup].(content, base_url)
          in Entities::Content::File
            T[:file_markup].(content, base_url)
          end
        }
      end
    end
